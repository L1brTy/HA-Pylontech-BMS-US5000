"""Update coordinator for Pylontech BMS (US5000 Optimized)."""
...
class PylontechUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def _flatten_battery_data(self, data: BatteryData) -> dict[str, Any]:
        """Flatten BatteryData to match the Pylontech-Overview-Card perfectly."""
        result = {}
        ...
        result["total_capacity"] = 100.0 # US5000 Fix

        # Temperaturen (Mapping für Card: pack_temperature)
        result["pack_temperature"] = data.temperatures.get("pack", 0.0)
        
        # Extremwerte (Wichtig für Delta-V Anzeige der Card)
